...
זוהי הפונקציה החשובה ביותר באפליקציה! כאשר ניר מצלמת תווית רכיבים בגב האריזה של מוצר (בין אם המוצר מוכר ובין אם הוא מוצר חדש, מיובא או כזה שלא מופיע באף קטלוג):
1. קרא וחלץ את כל רשימת הרכיבים בעברית או באנגלית בדיוק מירבי (OCR).
2. בצע בדיקה קלינית מדוקדקת של כל רכיב ורכיב לפי חוקי SIBO:
   ...
   - רכיבים בטוחים (כגון: עמילן תירס/מעובד E1422, אריתריטול, סטיביה/סטיביול גליקוזיד, תמצית וניל, מלח, שמן צמחי/שמן זית, קמח אורז, טפיוקה, קקאו, תפוחי אדמה, סודה לשתייה, אבקת אפייה, תבלינים טהורים) - מותרים לחלוטין!
3. חובת אישור המוצר (GREEN 🟢):
   - אם כל הרכיבים שנראים בתמונה בטוחים: קבע בוודאות status: "GREEN" (אור ירוק)!
   - אסור בתכלית לקבוע "מוצר לא מזוהה" או אור אדום כאשר הרכיבים נקראו והם בטוחים! אין שום משמעות לשאלה האם המוצר מופיע בקטלוג מראש או לא — בדיקת הרכיבים שבתמונה היא שקובעת ומאשרת את המוצר!
   - קבע foodName: שם המוצר שנראה על האריזה (למשל "אינסטנט פודינג וניל ללא סוכר סוויטנגו", "דפי אורז לבן", "קורנפלור", "תפוצ'יפס טבעי") או כותרת מתאימה.
...
`;
      ...
      const promptText = textPrompt
        ? `נתח את המאכל הבא עבור ניר עם סיבו (${isPhase1 ? 'שלב 1 קפדני' : 'שלב 2 הרחבה'}): "${textPrompt}". אם צורפה תמונה, קרא היטב את כל הטקסט ורשימת הרכיבים ונתח אותם.`
        : `קרא את כל הטקסט ורשימת הרכיבים המופיעים בתמונה (OCR). זהה מהו המוצר או המאכל שנמצא בתמונה (למשל: קורנפלור / עמילן תירס, אינסטנט פודינג וניל, שמן, דפי אורז, קמח, חטיף וכו'). נתח כל רכיב קלינית עבור ניר עם סיבו (${isPhase1 ? 'שלב 1 קפדני' : 'שלב 2 הרחבה'}). אם כל הרכיבים דלי FODMAP וללא טריגרים מתסיסים (עמילן תירס, סטיביה, אריתריטול, מים, מלח, אורז וכו') — קבע בוודאות אור ירוק GREEN! אל תקבע 'מוצר לא מזוהה' אם הטקסט או הרכיבים קריאים.`;
